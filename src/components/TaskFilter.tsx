...
import { fetchTasksData } from "../tasksThunks";
...
function TaskFilter() {
  ...
  const handleSearchForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    dispatch(fetchTasksData(searchInput, selectedValue));
  };

  const onChangeStatus = (e: ChangeEvent<HTMLSelectElement>) => {
    const value = e.target.value;
    setSelectedValue(value);
    dispatch(fetchTasksData(searchInput, value));
  };
  useEffect(() => {
  ...
  return (
    <form className="max-w-md mx-auto my-4 w-full" onSubmit={handleSearchForm}>
